...
无压缩取模工具：把 PNG/BMP/JPG/JPEG 图片转换为按目标像素格式逐像素打包的 `.bin`。输出可 O(1) 随机访问任意像素，是所有压缩算法体积率的对照基准（体积率恒 100%），也是唯一支持全部四种 Alpha 蒙版格式（`a8/a4/a2/a1`）的取模工具。
...
- 算法 nibble：`raw=0x0`（其余工具：`rle=0x1 imprle=0x2 qoi=0x3 indexqoi=0x4 qoif=0x5 indexqoimask=0x6`）
...
其他工具对这四种格式的支持情况：`img2bin_indexqoimask` 只支持 `a8`（压缩输出，与本节的行打包 payload 不同）；其余五个工具全部不支持——显式点名报参数错误（退出码 1），`--formats all` 自动跳过。
