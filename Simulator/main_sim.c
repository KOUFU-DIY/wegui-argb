...
#ifndef DEMO_ID /* autotest / CMake -DWE_DEMO_ID 可从外部覆盖 */
#define DEMO_ID (31)
#endif
...
#if (WE_SIM_AUTOTEST == 1) /* 回归测试脚手架，默认不编译，见 sim_autotest.h */
    sim_autotest_cfg_t autotest;

    /* 回归模式命令行解析必须早于 SDL 初始化（命中时切 dummy 视频驱动免开窗） */
    sim_autotest_parse_args(argc, argv, &autotest);
#endif
...
#warning "demo_showcase 按 800x480 布局编写，请把 we_user_config.h 的 SCREEN_WIDTH/SCREEN_HEIGHT 调大"
    ...
    we_gui_timer_create(&mylcd, &demo_timer, we_sevenseg_preview_demo_tick, 16U, 1U);
#elif (DEMO_ID == 118)
    ...
    we_gui_timer_create(&mylcd, &demo_timer, we_imgbtn_preview_demo_tick, 16U, 1U);
#elif (DEMO_ID == 121)
...
#if (WE_SIM_AUTOTEST == 1)
    /* headless 渲染回归：跑完 N 帧写出哈希即退出，不进交互主循环。 */
    if (autotest.frames > 0)
    {
        sim_autotest_run(&mylcd, &autotest, (int)(DEMO_ID));
        SDL_Quit();
        return 0;
    }
#endif
