...
5. 首字符步进宽即超 w 时仍收进本行（超宽字符独占一行，防死循环），越界像素部分被收窄后的 PFB 窗口裁掉；
6. 完整行高放不下的行不画；ellipsis 开启时最后一个可容纳行若还有剩余文本，按 "剩余可用宽 = w - 3×'.' 宽" 求最大前缀后追加 "..."。

## 渲染模型与成本
- 每行内容 memcpy 进栈缓冲补 '\0' 后交 `we_draw_string` 绘制；绘制期间 PFB 窗口收窄到自身矩形（marquee/group 同款 save/restore），越界像素自动裁掉
...
- **单字符宽度超 w**：该字符独占一行并照画，越出 w 的像素被 PFB 窗口裁掉（居中时向两侧越出）
- **空串**：不进折行循环，画面为空；连续 `\n\n` 产生空行（正常占行高）
...
- 行高恒为 `字体行高 + line_gap`，不随字形有效像素区高变化
