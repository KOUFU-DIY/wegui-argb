/**
 * @file  lite_autotest.c
 * @brief SimLite headless 渲染回归脚手架（自原 SDL 模拟器 sim_autotest.c 移植，
 *        哈希口径逐位兼容：帧缓冲布局与 565->ARGB8888 位扩展公式一致）
 */
...
static _lt_at_ev_t _lt_at_evs[_LT_AT_EV_MAX];

/**
 * @brief 对当前帧缓冲做 FNV-1a 链式哈希
 * @param h 传入：上一帧累计哈希（首帧传 2166136261u）
 * @return 融入本帧全部像素后的哈希值
 */
static uint32_t _lt_at_fb_hash(uint32_t h)
{
    const uint8_t *pb = (const uint8_t *)lite_fb();
 ...
 * @return 键值；无法识别返回 0
 */
static uint8_t _lt_at_key_from_name(const char *name)
{
    if (strcmp(name, "UP") == 0)
        return WE_KEY_UP;
 ...
 */
static int _lt_at_load_script(const char *path, const char **name_out)
{
    char line[96];
    ...
    *name_out = (bs != NULL && (fs == NULL || bs > fs)) ? (bs + 1)
    ...
    while (ev_cnt < _LT_AT_EV_MAX && fgets(line, sizeof(line), sf) != NULL)
    {
        char cmd[16];
        char karg[16];
        int frame;
        ...
        _lt_at_evs[ev_cnt].frame = (uint16_t)frame;
        _lt_at_evs[ev_cnt].x = (int16_t)x;
        _lt_at_evs[ev_cnt].y = (int16_t)y;
        if (strcmp(cmd, "down") == 0)
            _lt_at_evs[ev_cnt].cmd = _EV_DOWN;
        else if (strcmp(cmd, "move") == 0)
            _lt_at_evs[ev_cnt].cmd = _EV_MOVE;
        else if (strcmp(cmd, "up") == 0)
            _lt_at_evs[ev_cnt].cmd = _EV_UP;
        else
        {
            uint8_t k = 0U;

            if (sscanf(line, "%*d %*s %15s", karg) == 1)
                k = _lt_at_key_from_name(karg);
            if (k == 0U)
                continue;
            _lt_at_evs[ev_cnt].x = (int16_t)k;
            if (strcmp(cmd, "kinject") == 0)
                _lt_at_evs[ev_cnt].cmd = _EV_KINJ;
            else if (strcmp(cmd, "kpress") == 0)
                _lt_at_evs[ev_cnt].cmd = _EV_KPRESS;
            else if (strcmp(cmd, "krelease") == 0)
                _lt_at_evs[ev_cnt].cmd = _EV_KREL;
            else
                continue;
        }
        ev_cnt++;
 ...
 */
static void _lt_at_diagnostics(int frame, uint32_t crc)
{
...
void lite_autotest_parse_args(int argc, char *argv[], lite_autotest_cfg_t *cfg)
{
    ...
    /* headless：不开窗口，便于脚本批量跑；结果经文件回传保持与旧口径一致 */
    if (cfg->frames > 0)
        lite_set_headless(1);
}
...
void lite_autotest_run(we_lcd_t *lcd, const lite_autotest_cfg_t *cfg, int demo_id)
{
    ...
    if (cfg->script_path != NULL)
        ev_cnt = _lt_at_load_script(cfg->script_path, &script_name);
    ...
    for (f = 0; f < cfg->frames; f++)
    {
        ...
        for (e = 0; e < ev_cnt; e++)
        {
            if (_lt_at_evs[e].frame != (uint16_t)f)
                continue;
            switch (_lt_at_evs[e].cmd)
            {
            case _EV_DOWN:
                sim_touch.x = _lt_at_evs[e].x;
                sim_touch.y = _lt_at_evs[e].y;
            ...
            case _EV_KINJ:
                we_gui_key_inject(lcd, (uint8_t)_lt_at_evs[e].x);
                break;
            case _EV_KPRESS:
                we_gui_key_press(lcd, (uint8_t)_lt_at_evs[e].x);
                break;
            case _EV_KREL:
                we_gui_key_release(lcd, (uint8_t)_lt_at_evs[e].x);
                break;
        ...
        crc = _lt_at_fb_hash(crc);
        _lt_at_diagnostics(f, crc);
    }
